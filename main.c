...
/* -------------------------------------------------------------------------- */
		...
		{	if (over == 1)
			{printf("数据生成完毕！");}
			if (over != 1)
			{printf("数据处理发生错误!");break;}}
		if (langop == 1)
		{	if (over == 1)
			{printf("Data generation completed!");}
			if (over != 1)
			{printf("Data processing error!");break;}}
	}
	return 0;
}